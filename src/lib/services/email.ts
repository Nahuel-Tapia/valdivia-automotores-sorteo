...
/** Genera la plantilla HTML responsive para el correo del ticket digital */
function buildTicketEmailHTML(details: EmailOrderDetails): string {
  ...
  const ticketsBadges = details.tickets
    .map(
      (num) =>
        `<span style="display:inline-block; background-color:#f1f5f9; color:#1f2a52; border:1px solid #cbd5e1; font-weight:800; font-family:sans-serif; padding:6px 12px; margin:4px; border-radius:6px; font-size:14px; letter-spacing:1px;">${num}</span>`
    )
    .join(" ")
...
/** Envia el correo de confirmación utilizando Resend o loggea si no hay API key configurada */
