import{f as p}from"./cart.BaWbBNev.js";function o(){const r=document.getElementById("lookup-form"),l=document.getElementById("lookup-query"),t=document.getElementById("btn-lookup"),d=document.getElementById("results-section"),a=document.getElementById("results-container");r?.addEventListener("submit",async i=>{i.preventDefault();const n=l?.value.trim();if(n){t&&(t.disabled=!0,t.textContent="Buscando...");try{const s=await(await fetch(`/api/tickets/lookup?query=${encodeURIComponent(n)}`)).json();if(!s.success){alert(s.error||"No se pudo realizar la búsqueda.");return}if(d?.classList.remove("hidden"),s.orders.length===0){a.innerHTML=`
