...
function buildOrderSummary(details) {
  const ticketsBadges = details.tickets.map(
    (number) => `<span style="display:inline-block;background-color:#f1f5f9;color:#1f2a52;border:1px solid #cbd5e1;font-weight:800;font-family:Arial,sans-serif;padding:6px 12px;margin:4px;border-radius:6px;font-size:14px;letter-spacing:1px;">${escapeHtml(number)}</span>`
  ).join(" ");
  return `
...
function buildBaseEmail(title, body) {
  return `
    <!DOCTYPE html>
    <html lang="es">
    <head>
...
function buildPaymentRejectedEmailHTML(details) {
  return buildBaseEmail(
    "Pago rechazado",
    `
      <h2 style="color:#1f2a52;font-size:20px;margin-top:0;">Hola, ${escapeHtml(details.buyerName)}</h2>
...
async function sendPaymentRejectedEmail(details) {
  return sendEmail({
    to: details.buyerEmail,
...
export { sendPaymentRejectedEmail as a, sendOrderConfirmationEmail as s };
