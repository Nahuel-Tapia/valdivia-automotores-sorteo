...
import 'piccolore';
import { $ as $$, a as $$Layout, b as $$Lock, c as $$Ticket } from '../chunks/Layout_LZPCddmC.mjs';
...
}, "C:/Users/Docente/Documents/valdivia-automotores-sorteo/node_modules/lucide-astro/dist/Send.astro", void 0);
...
}, "C:/Users/Docente/Documents/valdivia-automotores-sorteo/node_modules/lucide-astro/dist/X.astro", void 0);
...
Ingresar al Panel
</button> </form> </div> </div>  <div id="dashboard-section" class="container-app hidden py-12 space-y-8"> <!-- Tarjetas de Métricas --> <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"> <div class="card p-5"> <div class="flex items-center justify-between"> <span class="text-xs font-700 uppercase tracking-wider text-navy/50">Recaudación Total</span> <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-brand/10 text-brand"> ${renderComponent($$result2, "DollarSign", $$DollarSign, { "class": "h-5 w-5" })} </span> </div> <p class="mt-3 font-display text-2xl font-800 text-navy" id="m-revenue">$0</p> <p class="mt-1 text-xs text-navy/60"><span id="m-approved-orders">0</span> órdenes aprobadas</p> </div> <div class="card p-5"> <div class="flex items-center justify-between"> <span class="text-xs font-700 uppercase tracking-wider text-navy/50">Boletos Vendidos</span> <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-green-500/10 text-green-600"> ${renderComponent($$result2, "Ticket", $$Ticket, { "class": "h-5 w-5" })} </span> </div> <p class="mt-3 font-display text-2xl font-800 text-navy" id="m-paid">0</p> <p class="mt-1 text-xs text-navy/60">Confirmados en DB</p> </div> <div class="card p-5"> <div class="flex items-center justify-between"> <span class="text-xs font-700 uppercase tracking-wider text-navy/50">Boletos Reservados</span> <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-amber-500/10 text-amber-600"> ${renderComponent($$result2, "Ticket", $$Ticket, { "class": "h-5 w-5" })} </span> </div> <p class="mt-3 font-display text-2xl font-800 text-navy" id="m-reserved">0</p> <p class="mt-1 text-xs text-navy/60">Tiempo en ventana de 15m</p> </div> <div class="card p-5"> <div class="flex items-center justify-between"> <span class="text-xs font-700 uppercase tracking-wider text-navy/50">Boletos Disponibles</span> <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-blue-500/10 text-blue-600"> ${renderComponent($$result2, "Ticket", $$Ticket, { "class": "h-5 w-5" })} </span> </div> <p class="mt-3 font-display text-2xl font-800 text-navy" id="m-available">200</p> <p class="mt-1 text-xs text-navy/60">De 200 totales</p> </div> </div> <!-- Tabla de Últimas Órdenes y Buscador --> <div class="card p-6"> <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-4"> <div> <h2 class="font-display text-lg font-700 text-navy">Gestión de Órdenes y Participantes</h2> <p class="text-xs text-navy/60">Tocá cualquier orden para ver los números asignados o re-enviar comprobantes</p> </div> <div class="relative w-full sm:w-64"> ${renderComponent($$result2, "Search", $$Search, { "class": "absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-navy/40" })} <input type="text" id="search-orders" placeholder="Buscar por DNI, Nombre o ID..." class="h-9 w-full rounded-xl border border-navy/15 pl-9 pr-3 text-xs text-navy focus:border-brand focus:outline-none"> </div> </div> <div class="overflow-x-auto"> <table class="w-full text-left text-sm"> <thead class="border-b border-navy/10 text-xs font-700 uppercase text-navy/50"> <tr> <th class="pb-3">ID Orden</th> <th class="pb-3">Comprador</th> <th class="pb-3">Email</th> <th class="pb-3">Boletos</th> <th class="pb-3">Monto</th> <th class="pb-3">Estado</th> <th class="pb-3 text-right">Acciones</th> </tr> </thead> <tbody id="orders-table-body" class="divide-y divide-navy/5 font-500"> <tr> <td colspan="7" class="py-4 text-center text-navy/50">Cargando órdenes...</td> </tr> </tbody> </table> </div> </div> </div>  <div id="order-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-navy/60 backdrop-blur-sm p-4"> <div class="relative w-full max-w-xl rounded-2xl bg-white p-6 shadow-2xl space-y-6"> <div class="flex items-center justify-between border-b border-navy/10 pb-4"> <div> <span class="text-xs font-700 uppercase text-brand">Ficha del Participante</span> <h3 id="modal-order-id" class="font-display text-xl font-800 text-navy">Orden #ORD-000000</h3> </div> <button type="button" id="modal-close-btn" class="rounded-lg p-1.5 text-navy/40 hover:bg-cloud hover:text-navy"> ${renderComponent($$result2, "X", $$X, { "class": "h-5 w-5" })} </button> </div> <!-- Datos del Comprador --> <div class="grid grid-cols-2 gap-4 text-xs"> <div class="rounded-xl bg-cloud/60 p-3"> <span class="text-navy/50 font-600 block">Nombre del Comprador</span> <strong id="modal-buyer-name" class="text-sm font-700 text-navy">--</strong> </div> <div class="rounded-xl bg-cloud/60 p-3"> <span class="text-navy/50 font-600 block">DNI / Documento</span> <strong id="modal-buyer-dni" class="text-sm font-700 text-navy">--</strong> </div> <div class="rounded-xl bg-cloud/60 p-3"> <span class="text-navy/50 font-600 block">Correo Electrónico</span> <strong id="modal-buyer-email" class="text-sm font-700 text-navy">--</strong> </div> <div class="rounded-xl bg-cloud/60 p-3"> <span class="text-navy/50 font-600 block">Teléfono / WhatsApp</span> <strong id="modal-buyer-phone" class="text-sm font-700 text-navy">--</strong> </div> </div> <!-- Estado y Pago --> <div class="flex items-center justify-between border-t border-b border-navy/10 py-3 text-xs"> <div> <span class="text-navy/50 font-600">Medio de Pago:</span> <strong id="modal-payment-method" class="ml-1 uppercase text-navy font-700">--</strong> </div> <div> <span class="text-navy/50 font-600">Monto Total:</span> <strong id="modal-total-amount" class="ml-1 text-sm font-800 text-brand">--</strong> </div> <div id="modal-status-badge"> <!-- Badge dinámico --> </div> </div> <!-- Grilla de Números de la Suerte Asignados --> <div> <h4 class="font-display text-xs font-700 uppercase tracking-wider text-navy/60 mb-2">Boletos de la Suerte Asignados</h4> <div id="modal-tickets-grid" class="flex flex-wrap gap-2 max-h-40 overflow-y-auto p-2 rounded-xl bg-cloud/50 border border-navy/10"> <span class="text-xs text-navy/40">Cargando boletos...</span> </div> </div> <!-- Acciones de Gestión --> <div class="flex flex-wrap items-center justify-between gap-3 pt-2"> <button type="button" id="modal-resend-email-btn" class="btn-secondary text-xs py-2.5 px-4 bg-brand/10 border-brand/20 text-brand hover:bg-brand hover:text-white transition font-700"> ${renderComponent($$result2, "Send", $$Send, { "class": "h-4 w-4 inline mr-1" })}
Re-enviar Comprobante por Email
</button> <div id="modal-action-approve-container"> <!-- Botón aprobar si está pending --> </div> </div> </div> </div> ` })} ${renderScript($$result, "C:/Users/Docente/Documents/valdivia-automotores-sorteo/src/pages/admin/index.astro?astro&type=script&index=0&lang.ts")}`;
